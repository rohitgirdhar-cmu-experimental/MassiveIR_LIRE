...
public class Indexer_SIFT {
    public static void main(String[] args) throws IOException {
        ...
        if (args.length > 1) {
            ...
            System.out.println("Indexing images in " + args[0] + " cluster into "
            		+ args[1] + " clusters");
            if (f.exists() && f.isDirectory()) passed = true;
        }
        if (!passed) {
            System.out.println("No directory given as first argument.");
            System.out.println("Run \"Indexer <directory> <numClusters>\" to index files of a directory.");
            System.exit(1);
        }
    ...
    public static String convertToLabel(String path, String dirPath) throws IOException {
        return path.substring((new File(dirPath).getCanonicalPath()).length() + 1);
    }
}
